...
source_id = "Southern District of Iowa Local Bankruptcy Rules"

sheet_links = {'Local Bankruptcy Rules': 'https://www.iasb.uscourts.gov/bankruptcy-rules'}

for name, main_link in sheet_links.items():
    try:
        response = requests.get(main_link)
        soup = BeautifulSoup(response.content, 'html.parser')
        all_links = soup.find('div',class_='field__item even').find_all('ul')[1].find('a')
        text = all_links.get_text(strip=True)
        link_url = 'https://www.iasb.uscourts.gov'+all_links.get("href")
